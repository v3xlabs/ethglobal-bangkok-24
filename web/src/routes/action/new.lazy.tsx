...
function RouteComponent() {
  ...
  const checkExpiry = async (name: string) => {
    const isExpiring = await checkNameExpiry(name);
    console.log(`${name} is expiring:`, isExpiring);
  };

  const getPrice = async (name: string) => {
    const price = await getNamePrice(name);
    console.log(`${name} price:`, price, "ETH");
  };
  ...
  const { mutate } = useMutation({
    ...
    mutationFn: async (data: { names: string[]; reward: number }) => {
      ...
      const result = await submitRenewalIntent(data.names, data.value);
  ...
  const form = useForm({
    ...
    onSubmit({ value }) {
      ...
      mutate(value.names, value.reward);
      ...
      <Card title="Create new action">
        ...
        >
          <form.Field
            ...
            validators={{
              onChange: ({ value }) => {
                if (value.length === 0) return "At least one name is required";
                return undefined;
              },
            }}
          >
              ...
              return (
                <div className="space-y-4">
                        ...
                        validators={{
                          ...
                          onChangeAsync: async ({ value }) => {
                            const result = await queryClient
                              .fetchQuery(lookupEnsNameQueryOptions(value))
                              .catch(() => {});
                          ...
                          return (
                            <div className="flex gap-2 items-center">
                              <Input
                                ...
                                onClickAction={() => field.removeValue(i)}
                                error={
                  ...
                  {field.state.value.length === 0 && (
                    <>
                      <Helper alert="error">No names added</Helper>
                    </>
                  )}
                  <Button
                    // className="w-fit"
          ...
          >
            {(field) => (
              <div className="space-y-2">
                ...
                />
                <Helper>
                  The reward paid to executors for renewing your names (in GWEI)
                </Helper>
              </div>
            )}
          </form.Field>
